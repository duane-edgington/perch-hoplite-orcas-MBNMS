...
**No TensorFlow is required, and this took work.** perch-hoplite's embedding database and search layer are TF-free upstream, but its linear classifier is not: `agile/classifier.py` imports TensorFlow at module level and builds the classifier in Keras. Rather than fork perch-hoplite, `phase2_classify.py` injects a minimal `tensorflow`/`tf.keras` stub so that module imports cleanly, and substitutes a PyTorch linear classifier (`pipeline/src/train.py`) for the Keras one. Everything TensorFlow touched in that module — `get_linear_model`, `bce_loss`, `hinge_loss`, `train_linear_classifier` — is replaced; `LinearClassifier` and `infer()` never used TensorFlow and are used as-is. The only other TensorFlow dependency, `zoo.model_configs`, is deliberately never imported. Of the thirteen perch-hoplite modules this pipeline imports, those two are the only ones that pull TensorFlow. Result: perch-hoplite installs from PyPI unmodified, and the pipeline runs pure PyTorch end to end. If you find yourself installing TensorFlow to run this, something has gone wrong — please open an issue.
